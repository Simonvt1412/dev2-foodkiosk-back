...
const supabaseClient_1 = require("../config/supabaseClient"); // Zorg dat het pad naar uw supabaseClient correct is
...
const loginUser = async (req, res) => {
    ...
    try {
        ...
        if (data.session && data.user) {
            ...
            const token = jsonwebtoken_1.default.sign({ userId: data.user.id, email: data.user.email, role: data.user.app_metadata?.role || data.user.user_metadata?.role || "user" }, JWT_SECRET, { expiresIn: "1h" } // Token verloopt na 1 uur
            );
            res.status(200).json({
